...
function exercise(project) {
  ...
  fs.watch(project, (evt, filename) => {
    try {
      ...
      const {birthtimeMs,ctimeMs,mtimeMs} = stat;
      console.log(birthtimeMs,':',ctimeMs,':',mtimeMs)
      if (!stat.isDirectory() && stat.birthtimeMs===stat.ctimeMs&& stat.birthtimeMs===stat.mtimeMs ) {
        answer = filepath
      }
      // answer = filepath
    } catch (err) {
